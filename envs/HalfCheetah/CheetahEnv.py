...
class ExtendedCheetah():
    ...
    def reset(self, uState, attributes=None):
        # Attributes: [state_filter, reward_filter, total_true_reward]
        # uState has dim 18, filtered state has dim 17
        state = self.custom_env.reset(uState, attributes, name="Cheetah")
        return state
...
class HalfCheetah:
    def __init__(self, model="PPO"):
        ...
        if model == "PPO":
            ...
            self.attack_checkpoint = torch.load("../envs/HalfCheetah/HalfCheetah_Attack_PPO.model")
            ...
            self.attack_checkpoint = torch.load("../envs/HalfCheetah/HalfCheetah_Attack_ATLA.model")
            self.model = CtsPolicy(17, 6, "orthogonal")
            self.attack_model = CtsPolicy(17, 17, "orthogonal")
        elif model == "LSTM":
            self.checkpoint = torch.load("../envs/HalfCheetah/HalfCheetah_LSTM.model")
            self.attack_checkpoint = torch.load("../envs/HalfCheetah/HalfCheetah_Attack_LSTM.model")
        ...
        self.env = ExtendedCheetah(self.model, self.custom_env, self.attack_model, self.eps)
