...
from InvertedPendulumEnv import InvertedPendulum
...
''')
    ...
    if params['sarsa_enable']:
        ...
        for i in range(num_episodes):
            ...
            ep_length, ep_reward, actions, action_means, states, kl_certificates = p.run_test(max_len=200, compute_bounds=params['compute_kl_cert'], use_full_backward=params['use_full_backward'], original_stdev=None)
...
def test_network(p, num_episodes, num_iterations):
    all_rewards = []
    all_lens = []
    for i in range(num_episodes):
        print('Episode %d / %d' % (i+1, num_episodes))
